...
class DPOTrainer(ABC):
    ...
    def __init__(
        ...
        self.beta = beta
        self.loss_fn = DPOLoss(self.beta, self.args.label_smoothing, self.args.loss_type)
    ...
    def _get_batch_logps(
        ...
        if self.args.loss_type == "simpo":
            logprobs_sums = logprobs_sums / loss_masks.sum(-1)
        logprobs_means = (per_token_logps * loss_masks).sum(-1) / loss_masks.sum(-1)
        return logprobs_sums, logprobs_means
    
    def calculate_implicit_reward(self, output_file):
        self.model.eval()
        if not self.args.loss_type == "simpo":
            self.ref_model.eval()
        
        local_output_file = f"{output_file}{self.strategy.get_rank()}"
        
        step_bar = tqdm(
            range(self.train_dataloader.__len__()),
            desc="Calculate implicit reward",
            disable=not self.strategy.is_rank_0(),
        )
        ...
        for data in self.train_dataloader:
            if not self.packing_samples:
                ...
                rejected_texts = self.tokenizer.batch_decode(reject_ids, skip_special_tokens=True)
                
                for i in range(len(chosen_texts)):
                    if not self.args.loss_type == "simpo":
                        chosen_reward = chosen_logps[i].cpu().item() - reference_chosen_logps[i].cpu().item()
                        rejected_reward = rejected_logps[i].cpu().item() - reference_rejected_logps[i].cpu().item()
                        result = {
                            'chosen_text': chosen_texts[i],
                            'rejected_text': rejected_texts[i],
                            'chosen_logp': chosen_logps[i].cpu().item(),
                            'rejected_logp': rejected_logps[i].cpu().item(),
                            'ref_chosen_logp': reference_chosen_logps[i].cpu().item(),
                            'ref_rejected_logp': reference_rejected_logps[i].cpu().item(),
                            'reward': chosen_reward - rejected_reward
                        }
                    else:
                        chosen_reward = chosen_logps[i].cpu().item()
                        rejected_reward = rejected_logps[i].cpu().item()
                        result = {
                            'chosen_text': chosen_texts[i],
                            'rejected_text': rejected_texts[i],
                            'chosen_logp': chosen_logps[i].cpu().item(),
                            'rejected_logp': rejected_logps[i].cpu().item(),
                            'reward': 2.5 * (chosen_reward - rejected_reward) - 0.6
                        }
        ...
        if self.strategy.is_rank_0():
            results = []
            for rank in range(self.strategy.world_size):
                local_file = f"{output_file}{rank}"
                with jsonlines.open(local_file, 'r') as reader:
